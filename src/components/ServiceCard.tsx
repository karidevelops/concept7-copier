...
const ServiceCard = ({ title, description, price, buttonText, onButtonClick, isPopular, detailsType, details }: ServiceCardProps) => {
      ...
      <div className="relative h-[420px]" style={{ perspective: "1000px" }}>
        ...
        >
          ...
          <Card className="absolute inset-0 flex flex-col h-full hover:shadow-2xl transition-shadow duration-300 border border-border bg-card group overflow-visible" style={{ backfaceVisibility: "hidden" }}>
            {isPopular && (
              <div
                className="absolute -top-3 left-1/2 -translate-x-1/2 bg-accent text-accent-foreground px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wide shadow-lg flex items-center gap-2 z-10 animate-[pulse_3s_ease-in-out_infinite]"
                style={{
                  transform: isFlipped ? "rotateY(180deg)" : "rotateY(0deg)",
  ...
  return (
    <Card className="p-8 flex flex-col h-full hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 border border-border bg-card group relative overflow-visible">
      {isPopular && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-accent text-accent-foreground px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wide shadow-lg flex items-center gap-2 z-10 animate-[pulse_3s_ease-in-out_infinite]">
          <Star className="h-4 w-4 fill-current" />
          Suosituin
        </div>
      )}
      ...
      <h3 className="text-2xl font-bold text-foreground mb-4 group-hover:text-accent transition-colors duration-300 uppercase tracking-wide break-words whitespace-pre-line">
        {title}
      </h3>
